...
(defvar my/mu4e-dashboard-buffer-name "mu4e-dashboard.org"
  "Name des mu4e-dashboard Buffers. Passe das an, falls dein Dashboard anders heißt.")

;; --- Display-Regel: Dashboard IMMER links ------------------------------------
...
;; --- Dashboard erzeugen/anzeigen ---------------------------------------------
(defun my/mu4e-dashboard-open ()
  "Öffne oder aktualisiere das mu4e-dashboard im linken Side-Window."
  (interactive)
  ;; sicherstellen, dass der Buffer existiert
  (unless (get-buffer my/mu4e-dashboard-buffer-name)
    ;; Dashboard initial erzeugen, aber Layout nicht zerstören
...
;; --- Mail Workspace (Slot 1) -------------------------------------------------
(defun my/mu4e-mail-workspace ()
  "Erstelle oder repariere das Mail-Layout (Slot 1)."
  (interactive)
  ...
  (add-hook 'mu4e-main-mode-hook
            (lambda ()
              (message "mu4e gestartet – öffne Dashboard...")
...
;; --- Hook: beim Wechsel auf Slot 1 immer Dashboard prüfen --------------------
(defun my/eyebrowse-mail-dashboard-setup ()
  (when (= (eyebrowse--get 'current-slot) 1)
    (my/mu4e-dashboard-open)))
(add-hook 'eyebrowse-post-window-switch-hook #'my/eyebrowse-mail-dashboard-setup)

;;;; --- Initiales Layout (dein ursprüngliches Setup, nur robuster) ------------

(defun my-initial-window-setup ()
  ;; Slots benennen wie gehabt
  (eyebrowse-switch-to-window-config-1)
  (eyebrowse-rename-window-config 1 "Mail")
  ...
  ;; Statt (mu4e) + (my/mu4e-dashboard-left) direkt das robuste Setup:
