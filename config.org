...
(global-hl-line-mode 0)
  ...
  #+end_src
** Text scaling
 Replaced the default-text-scale package with the built-in command
 =global-text-scale-adjust=, bound out of the box to C-x C-M-+ / C-x C-M-- /
 C-x C-M-0. (The package's own C-M-= / C-M-- / C-M-0 were never active here,
 because =default-text-scale-mode= was never enabled.)
 The shorter keys are re-bound below so they now actually do something.
 #+begin_src emacs-lisp
(global-set-key (kbd "C-M-=") 'global-text-scale-adjust)
(global-set-key (kbd "C-M--") 'global-text-scale-adjust)
(global-set-key (kbd "C-M-0") 'global-text-scale-adjust)
 #+end_src
** Workspaces (tab-bar)
 I am using named workspaces which I switch between with F6-F12.
 This replaces eyebrowse (archived upstream) with the built-in =tab-bar-mode=.

 Note that tab-bar addresses tabs by *position*, while eyebrowse addressed them
 by a stable slot number. To keep the eyebrowse feel, the F-keys below switch by
 *name* and create the workspace on demand, so closing one does not shift the
 others around.
 #+begin_src emacs-lisp
(use-package tab-bar
  :ensure nil ; built-in since Emacs 27
  :init
  (setq tab-bar-show nil                  ; no visible bar, the F-keys are enough
        tab-bar-new-tab-choice "*scratch*"
        tab-bar-close-button-show nil
        tab-bar-tab-hints t)
  :config
  (tab-bar-mode 1)
  (tab-bar-history-mode 1))               ; per-tab window undo: C-c <left>/<right>

(defun cu/tab-names ()
  "Names of all existing workspaces (tabs) in the selected frame."
  (mapcar (lambda (tb) (alist-get 'name tb)) (tab-bar-tabs)))

(defun cu/switch-to-workspace (name)
  "Switch to the workspace called NAME, creating it if it does not exist.
This mimics `eyebrowse-switch-to-window-config', which also created the
slot on demand instead of clamping to the existing range."
  (interactive (list (completing-read "Workspace: " (cu/tab-names))))
  (if (member name (cu/tab-names))
      (tab-bar-select-tab-by-name name)
    (tab-bar-new-tab)
    (tab-bar-rename-tab name)))

;; Same key -> same workspace as the old eyebrowse setup.
;; NOTE: written out instead of generated in a loop - config.el is loaded with
;; dynamic binding, so a lambda closing over a loop variable would not work.
(global-set-key (kbd "<f12>") (lambda () (interactive) (cu/switch-to-workspace "Mail")))
(global-set-key (kbd "<f11>") (lambda () (interactive) (cu/switch-to-workspace "Org")))
(global-set-key (kbd "<f10>") (lambda () (interactive) (cu/switch-to-workspace "Code")))
(global-set-key (kbd "<f9>")  (lambda () (interactive) (cu/switch-to-workspace "Docs")))
(global-set-key (kbd "<f8>")  (lambda () (interactive) (cu/switch-to-workspace "Misc")))
(global-set-key (kbd "<f7>")  (lambda () (interactive) (cu/switch-to-workspace "Comm")))
(global-set-key (kbd "<f6>")  (lambda () (interactive) (cu/switch-to-workspace "Dev")))
 #+end_src
** Projectile
#+begin_src emacs-lisp
...
** mqttclient-mode
 ...
 #+end_src
** json
 Using the built-in =json-ts-mode= instead of the json-mode package.
 Note: the json-mode keymap (C-c C-f beautify, C-c C-p show-path, C-c C-t
 toggle-boolean, ...) does not exist here. =json-pretty-print-buffer= comes
 from built-in json.el and still works.
 #+begin_src emacs-lisp
...
* dockerfile
Using the built-in =dockerfile-ts-mode=. Note this drops the build commands of
the dockerfile-mode package (C-c C-b dockerfile-build-buffer, C-c M-b
dockerfile-build-no-cache-buffer) - the docker package (C-c d) covers builds.
#+begin_src emacs-lisp
...
(use-package consult
  ...
  :bind (("M-y"   . consult-yank-pop)      ; replaces kill-ring-search
         ("M-s r" . consult-ripgrep)       ; replaces ag
         ("M-s g" . consult-grep)
         ("M-s l" . consult-line)
...
#+end_src
* Searching
Replaced the ag package with =consult-ripgrep= Bindings live in the consult section:
=M-s r= project-wide ripgrep, =M-s g= grep, =M-y= kill-ring browser
(replaces the kill-ring-search package).
* Git handling
...
; prettify json responses in verb buffers
(defun verb-json-response-mode-hook ()
              (json-ts-mode)
...
(with-eval-after-load 'ox-latex
  ...
  (setq org-latex-default-packages-alist '((#1="" "amsmath" t ("lualatex" "xetex"))
                                           (#1# "fontspec" t ("lualatex" "xetex"))
                                           (#1# "graphicx" t) (#1# "longtable" nil) (#1# "wrapfig" nil)
                                           (#1# "rotating" nil) ("normalem" "ulem" t)
                                           (#1# "amsmath" t ("pdflatex")) (#1# "amssymb" t ("pdflatex"))
                                           (#1# "capt-of" nil) (#1# "hyperref" nil)))
  )

;; NOTE: org-ai, org-roam, org-roam-ui, org-superstar and org-super-agenda are
;; configured in their own sections further below
...
(defvar my/mu4e-dashboard-buffer-name "mu4e-dashboard.org"
  "Name of mu4e-dashboard buffer. (customize if necessary)")

;; --- Display-rule: Dashboard always left ------------------------------------
(setq display-buffer-alist
      (cons
       `(,(concat "\\`" (regexp-quote my/mu4e-dashboard-buffer-name) "\\'")
...
(defun my/mu4e-dashboard-open ()
  "Open or update mu4e-dashboard in side-window."
  (interactive)
  ;; assure buffer exists
  (unless (get-buffer my/mu4e-dashboard-buffer-name)
    ;; initial create dashboard, but do not kill layout
    (save-window-excursion
      (if (fboundp 'mu4e-dashboard)
          (mu4e-dashboard)
        (message "mu4e-dashboard not found"))))
  ;; get buffer
  (let ((buf (get-buffer my/mu4e-dashboard-buffer-name)))
    (when buf
...
;; --- Mail Workspace ----------------------------------------------------------
(defun my/mu4e-mail-workspace ()
  "Create of repair mail layout in workspace \"Mail\"."
  (interactive)
  (cu/switch-to-workspace "Mail")
  (delete-other-windows)
  (message "Starting mu4e...")
  (add-hook 'mu4e-main-mode-hook
            (lambda ()
              (message "mu4e startet – opening dashboard...")
              (my/mu4e-dashboard-open))
            ;; nur einmal
            :once)
  (mu4e))

;; --- Hook: Changing to "Mail" -> check dashboard --------------------
(defun my/mail-workspace-dashboard-setup (&rest _)
  "Re-open the mu4e dashboard whenever the \"Mail\" workspace is selected."
  (when (equal (alist-get 'name (tab-bar--current-tab)) "Mail")
    (my/mu4e-dashboard-open)))
(add-hook 'tab-bar-tab-post-select-functions #'my/mail-workspace-dashboard-setup)

;;;; --- Initiales Layout ------------------------------------------------------

(defun my-initial-window-setup ()
  ;; Rename startiong tab to "Mail"
  (tab-bar-rename-tab "Mail")
  ;; create and rename the remaining ones 
  (cu/switch-to-workspace "Org")
  (cu/switch-to-workspace "Code")
  (cu/switch-to-workspace "Docs")
  (cu/switch-to-workspace "Misc")

  (cu/switch-to-workspace "Mail")
  (delete-other-windows)

  (my/mu4e-mail-workspace))

(org-toggl-set-workspace "Christoph's workspace")
(add-hook 'emacs-startup-hook #'my-initial-window-setup)
#+end_src
